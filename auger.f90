program dirac
  ...
  logical                                      :: s1, s2
  complex(kind=dp), dimension(:,:), allocatable :: wvfn
...
contains
  ...
  ! and return the L'th term of the expansion i.e L = 1 for dipole
  function coulomb_laplace_expansion(r1, r2, L) result(op)
    implicit none

    real(kind=dp), dimension(3), intent(in)  :: r1, r2
    integer,                     intent(in)  :: L

    real(kind=dp)                            :: op, total, rmin, rmax

    complex(kind=dp) :: Y1, Y2

    integer :: M

    ! rmin = min(norm2(r1), norm2(r2))
    ! rmax = max(norm2(r1), norm2(r2))
    rmin = min(sqrt(dot_product(r1,r1)), sqrt(dot_product(r2,r2)))
    rmax = max(sqrt(dot_product(r1,r1)), sqrt(dot_product(r2,r2)))
    total = 0.0_dp
    ! Perform the sum over M
    do M = -L, L
      Y1 = SphericalYCartesian(L,-M, r1)
      Y2 = SphericalYCartesian(L,M, r2)
      total = total +(-1.0_dp)**M * rmin**L / rmax**(L+1) * Y1 * Y2
    end do
    
    op = total * 4.0_dp * pi / (2.0_dp * L + 1.0_dp)
     

  end function coulomb_laplace_expansion
  ...
  subroutine calculate_schrodinger_dipole(initial_state, final_state, transition_matrix)
    implicit none
    ...
    transition_matrix%total_rate = rate
  end subroutine
